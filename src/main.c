...
int main(int argc, char** argv) {
  ...
  if(world_rank == 0){
    ...
    // Generar 3 caches
    Cache* cachePool[3];
    cachePool[0] = newCache(10); // Algoritmo 1: Armstrong
    cachePool[1] = newCache(10); // Algoritmo 2: Euler
    cachePool[2] = newCache(10); // Algoritmo 3: Configuracion electronica
    ...
    while(1){
      ...
      while(done == 0){ // solo si quedan queries sin respuesta
        query = getUnassignedQuery(queries);
